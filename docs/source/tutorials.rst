...
.. grid:: 1
   ...
   .. grid-item-card:: Tutorial 7 — DistributedAnalysis over PyAEDT (gRPC), no COM
      :link: _tutorial_notebooks/Tutorial 7. DistributedAnalysis over PyAEDT (gRPC) — no COM.html
      :link-type: url

      :bdg-warning:`Ansys HFSS required`

      Run pyEPR's normal ``DistributedAnalysis`` over Ansys's official PyAEDT gRPC API — no COM — by passing ``use_pyaedt=True`` to ``ProjectInfo``. Matches the COM path digit-for-digit.
...
.. toctree::
   ...
   _tutorial_notebooks/Tutorial 7. DistributedAnalysis over PyAEDT (gRPC) — no COM
